...
**Upload an ambiguous requirements document — get back a reviewed, standards-compliant High-Level Design (HLD).** Vishwakarma interrogates requirements the way a senior architect would: every quality attribute surfaced and cited to the sentence that implies it, every open decision resolved through scored trade-offs, and the finished design reviewed and signed off like a pull request. Technical track for architects today; a functional track for business analysts in Phase 2.
...
> **Context.** Built for the AI/Works hackathon. Stage one of that lifecycle names *Requirements Capture and Enrichment* but supplies no document standard, no decision record, and no human sign-off workflow — Vishwakarma builds exactly that layer. [Section 2](#2-aiworks-ecosystem-integration) covers the integration in full.
...
Ambiguous requirements don't fail at requirements time. They fail months later — in UAT, or in production — once the architectural options have narrowed and the cost of changing course has compounded. What's missing at the point of definition is structure: no agreed document standard, no record of which decisions were made or why, and no sign-off before those requirements flow downstream into design, specs, and code.
...
2. **Generate** — the complete **High-Level Design** (arc42 v9 · C4+ADR · RFC) streamed live, grounded in the characteristics and decisions recorded above. **Functional documents** (BRD · FRD · User Story Map) follow the same pipeline for business analysts. *(Phase 2)*
...
Vishwakarma was built for stage one of the [AI/Works delivery lifecycle](https://www.thoughtworks.com/ai/works/technical-guide). Instead of downstream spec generation inheriting raw client documents with every ambiguity intact, what reaches it is a structured, validated, approved HLD — every quality attribute ranked, every decision recorded, signed off by both business and technical reviewers. [Section 2](#2-aiworks-ecosystem-integration) has the full integration diagram.
...
5. [The Generated HLD](#5-the-generated-hld)
...
**Vishwakarma turns an ambiguous requirements document into a reviewed, standards-compliant High-Level Design.** Before a single architecture section is written, it reads the requirements the way an experienced architect would — critically. It surfaces every quality attribute buried in business prose, exposes every unresolved trade-off, and walks the architect through every open decision with scored options and explicit consequences. Nothing is assumed. Nothing is glossed over.
...
- **Threaded comments** — replies, resolutions, and re-openings tracked; every comment anchored to the HLD version it references
- **Structured approval lifecycle**: `Draft → Submitted for Review → In Review → Changes Requested → Approved`
- **Version tracking on every HLD** — every generation, edit, and AI-assisted revision creates a named version snapshot; versions are comparable
...
The entire enrichment pipeline — interrogated characteristics, resolved decisions, approved HLD — is structured specifically to serve as the **validated handoff contract into Dynamic Spec**. Dynamic Spec no longer inherits ambiguity. It receives architecture that has already been interrogated, decided, reviewed, and approved.

---

### The Two Delivery Artifacts

| Artifact | Audience | Standard | Status |
|---|---|---|---|
| **High-Level Design (HLD)** | Architects, engineers | arc42 v9 · C4 + ADR · RFC · Custom | ✅ **Phase 1 — Built** |
| **Functional docs** | Business analysts, product owners | Business-aligned · Plain English · Quality attributes as functional criteria | 🔄 **Phase 2** |
...
flowchart TD
    ...
    subgraph VISH["VISHWAKARMA - Requirements to Reviewed HLD"]
...
When Dynamic Spec consumes a Vishwakarma HLD, it receives a document that has already:
...
| HLD version and approval history | Non-existent | Full version history + approval trail committed to source control |
| Downstream AI agent quality | Constrained by raw, ambiguous input | Structured, validated, approved HLD as input |
...
Traditional Architecture Review Boards are slow, centralised, and — as Thoughtworks Architecture Advice Process research confirms — **correlated with low organisational performance**. Vishwakarma's versioned HLD with inline review and approval is the infrastructure that makes decentralised architectural governance safe: teams decide, the Decision Ledger captures it, the approval workflow validates it.
...
| **Compliance** | Architecture decisions distributed across email, Confluence, Slack, tribal memory | Single, versioned, approved, exportable HLD that satisfies architecture governance requirements |
...
| **Lightweight ADRs** | Thoughtworks Tech Radar — Adopt (graduated) | [architectural-decision-making.md](references/architectural-decision-making.md) | First-class output in every HLD; noun-phrase titles, active-voice decisions, split consequences, immutable once accepted |
...
| **Observability — OpenTelemetry** | Thoughtworks Tech Radar — Adopt | [observability.md](references/observability.md) | All three pillars — logs, metrics, traces — required in generated HLDs with a named approach |
...
A **High-Level Design** is neither a summary nor a template fill. It is the output of a structured interrogation — ambiguities resolved, decisions explicit, standards applied, every output element traceable to source. The HLD is a living, versioned artifact with a full review and approval lifecycle.

---

### High-Level Design (HLD) *(Phase 1 — Built)*
...
Every HLD includes: framework sections derived from the pipeline · ADRs with trade-off cards · C4 diagrams in Mermaid v11 with ELK auto-layout · Live Presentation Mode · inline review and commenting · version tracking · approval workflow · change history · ZIP export (source-control ready).

---

### Functional Documents *(Phase 2)*
...
| HLD — arc42 v9 (12 sections, section-level rules enforced) | ✅ Built |
| HLD — C4 + ADR (strict C4 schema + Fowler ADR format) | ✅ Built |
| HLD — RFC format | ✅ Built |
| HLD — Custom section structure | ✅ Built |
...
| **HLD version tracking** — named snapshots on every change; versions comparable | ✅ Built |
...
| Functional track — BRD · FRD · User Story Map with plain-English diagrams | 🔄 Phase 2 |
...
|---|---|
| **Human-in-the-loop at every stage** | Architect confirms characteristics, ranks priority, selects interview answers. No characteristic or decision reaches the next stage without explicit confirmation. Approval workflow extends this to all reviewers — no HLD version is final without explicit sign-off. |
...
| **Full output traceability** | Every section of the generated HLD traces back to a confirmed characteristic and a Decision Ledger entry. The architecture is derived — not generated. |
...
The complete end-to-end flow from requirements document to approved architecture. Each screenshot below shows a distinct stage in the pipeline — the journey from raw ambiguous text to a reviewed, version-stamped, approved High-Level Design.
...
Four standards-aligned frameworks are available once all decisions are resolved. The selection determines the structure, section rules, and documentation standard applied to the generated HLD. The Custom option preserves existing team standards.

![Documentation Framework Selection — arc42, C4+ADR, RFC, Custom](images/image-9.png)

Custom section structure editor — define exactly which sections the HLD should contain.

![Custom Section Structure Editor](images/image-8.png)

---

### Step 5 — Generated HLD with Inline Editing

The complete HLD is generated in real time, section by section via live streaming. Architecture Decision Records with full trade-off cards are generated inline. Any section can be edited directly or revised through conversational AI prompts without regenerating the full document.

![Generated HLD — Document View with Sections](images/image-12.png)
...
### Step 8 — Submit HLD for Architectural Review

Once the author is satisfied, the HLD is submitted for review. The reviewer receives a direct link to the specific HLD version — no email thread, no shared editing, no version confusion.

![Submit HLD for Architectural Review](images/image-18.png)

---

### Step 9 — Reviewer Dashboard with Version History

The reviewer dashboard shows every HLD pending sign-off, the version it is currently at, and the complete version history of how the document evolved. Reviewers see exactly what changed between versions and which comment triggered each revision.

![Reviewer Dashboard — HLD Pending Sign-off with Version History](images/image-19.png)
...
![HLD Inline Review — Section-level Threaded Comments](images/image-21.png)
...
### Step 11 — Phase 2 Preview — Functional Documents
...
![Functional Documents — Phase 2 Preview](images/image-24.png)
